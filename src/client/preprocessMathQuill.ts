...
function findClosingBrace(str: string, startIdx: number) {
 ...
 */
export function mathquillToMathJS(fromMQ: string) {
  const replacements = [
 ...
 */
function convertSubscript(expr: string): string {
  const sub = '_{';
 ...
 */
function fracToDivision(expr: string): string {
  const frac = '\\frac';
